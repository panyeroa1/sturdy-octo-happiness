...
import { CONFIG, AUTOSTART } from "./config/config.js"
...
if (screenPreview) screenPreview.playsInline = true
// Video preview element
const videoPreview = document.getElementById("preview")
if (videoPreview) videoPreview.playsInline = true
...
async function connectToWebsocket() {
  const config = {
    ...
    // ✅ Enable STT (input) + output transcription
...
// ✅ Show what the user said (STT)
client.on("input_transcription", (t) => {
...
// ✅ Show what the model spoke (output transcription of the TTS audio)
client.on("output_transcription", (t) => {
  const text = (t && t.text) ? String(t.text).trim() : "";
  if (!text) return;
  logMessage(text, "ai");
  lastAssistantMessage += text;
...
// === PostMessage API and Embed Support ===

window.addEventListener("message", async (event) => {
  const msg = event.data
  if (!msg || msg.type !== "orbit_translator_command") return

  const { command, payload } = msg
  console.log("[OrbitTranslator] Received command:", command, payload)

  switch (command) {
    case "start":
      if (!isConnected) {
        logMessage("Starting via embed command...", "system")
        await connectToWebsocket()
      }
      setTimeout(async () => {
    ...
    case "shutdown":
      if (isConnected) {
        logMessage("Shutting down via embed command...", "system")
        disconnectFromWebsocket()
      }
      break
...
`.trim();
        ...
        systemInstructionInput.value = base;
        updateConfiguration(); // will reconnect safely
      }
      break;
  }
})
...
// Auto-start check
if (AUTOSTART) {
  logMessage("Auto-starting Orbit Translator...", "system")
  setTimeout(async () => {
    if (!isConnected) {
      await connectToWebsocket()
      // Wait for connection/audio init
      setTimeout(() => {
        if (!isRecording) handleMicToggle()
      }, 1000)
    }
  }, 500)
}
